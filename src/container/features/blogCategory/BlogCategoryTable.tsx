...
import {
  selectCreateBlogCategoryData,
  selectDeleteBlogCategoryData,
  selectGetBlogCategoryData,
  selectGetBlogCategoryLoading,
  selectUpdateBlogCategoryData,
...
interface BlogCategoryTypes {
  onRowClick?: any;
}

const BlogCategoryTable: React.FC<BlogCategoryTypes> = (props) => {
  ...
  const loading = useSelector(selectGetBlogCategoryLoading);
  const blogCategoryData = useSelector(selectGetBlogCategoryData);
  const updateData = useSelector(selectUpdateBlogCategoryData);
  const createData = useSelector(selectCreateBlogCategoryData);
  const deleteData = useSelector(selectDeleteBlogCategoryData);

  useEffect(() => {
    dispatch(GetBlogCategoryAction({ page: 0, size: 999 }));
  }, []);
  ...
  const handleFilterParameters = (data: any) => {
    const { FoodName, BlogCategory, Restaurant } = data;
    let queryParam = "";
    if (FoodName) queryParam += "title=" + FoodName + ",";
    if (BlogCategory?.label)
      queryParam += "categoriesId=" + BlogCategory?.value + ",";
    if (Restaurant?.label)
  ...
  return (
    <CollectionControls
      buttons={["create"]}
      createTitle="ساخت دسته بندی مقاله جدید"
      hasBox={false}
    ...
    >
      <Table className="w-full" isLoading={false} shadow={false}>
        <TableHead className="w-full" isLoading={false} shadow={false}>
          <TableRow>
            <TableHeadCell>نام دسته بندی مقاله </TableHeadCell>
            <TableHeadCell />
          </TableRow>
        </TableHead>
        <TableBody>
