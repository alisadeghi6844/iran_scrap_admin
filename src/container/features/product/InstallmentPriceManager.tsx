...
const InstallmentPriceManager: React.FC<InstallmentPriceManagerProps> = ({
  ...
  const currentPrices =
    installmentPrices && installmentPrices.length > 0
      ? installmentPrices
      : [{ duration: 1, price: 0 }];
  // اضافه کردن یک سطر جدید
  const addNewRow = () => {
    if (currentPrices.length < 12) {
      // حداکثر 12 ماه
      const newRow = { duration: currentPrices.length + 1, price: 0 };
  ...
  const removeRow = (index: number) => {
    if (currentPrices.length > 1) {
      ...
      // بروزرسانی duration ها بر اساس index جدید
