...
import {
  GET_AUTH_HISTORY_POINT,
  GET_USERS_POINT,
  GET_USERS_PROVIDERS_POINT,
  GET_USERS_STATISTIC_POINT,
...
export const GetAuthHistoryService = async (query: any) => {
  let queryText;
  if (!!query) {
    queryText = AxiosQueryCustom(query);
  }
  return await HttpServises.get(
    `${BASE_URL}${GET_AUTH_HISTORY_POINT}?${queryText ? queryText : ""}`
  );
};

export const GetUsersStatisticService = async (query: any) => {
  let queryText;
  if (!!query) {
    queryText = AxiosQueryCustom(query);
  }
  return await HttpServises.get(
    `${BASE_URL}${GET_USERS_STATISTIC_POINT}?${queryText ? queryText : ""}`
  );
};
